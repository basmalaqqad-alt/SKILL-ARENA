...
const AITab = ({ userName = 'Hero' }) => {
  ...
  const [chatMessages,      setChatMessages]      = useState([{
    role: 'ai',
    text: `Hi ${userName}! 👋 I'm your AI learning assistant. Ask me anything about your courses or learning path!`,
  }]);
  ...
  useEffect(() => { if (profile) generateAIRecommendation(); }, [profile]); // eslint-disable-line
  ...
  const generateAIRecommendation = async () => {
    ...
    try {
      ...
      const inProg    = profile?.in_progress_courses ?? [];

      const prompt =
        'You are an AI learning advisor for SkillArena.\n\n' +
        'Student profile:\n' +
        `- Name: ${userName}\n- XP: ${xp}\n- Rank: ${rank}\n` +
        `- Completed: ${completed.map(c => c.title).join(', ') || 'None yet'}\n` +
        `- In progress: ${inProg.map(c => `${c.title} (${c.progress}%)`).join(', ') || 'None'}\n\n` +
        'Give a SHORT motivating personalized recommendation (3-4 sentences):\n' +
        '1. One specific next step\n2. Motivational insight about rank\n3. Suggested focus area\n\n' +
        'Be friendly, energetic, use emojis. English only.';

      const result = await askGroq(prompt, 'You are a helpful learning advisor.');
      setAiRecommendation(result);
    } catch {
      setAiRecommendation('Unable to load AI recommendations. Please try again.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const xp     = profile?.experience ?? 0;
      const rank   = profile?.rank_name || 'Novice';
      const comp   = profile?.completed_courses ?? [];
      const inProg = profile?.in_progress_courses ?? [];

      const prompt =
        'You are a friendly AI learning assistant on SkillArena.\n\n' +
        `Student: ${userName}, XP: ${xp}, Rank: ${rank}\n` +
        `Completed: ${comp.map(c => c.title).join(', ') || 'None'}\n` +
        `In progress: ${inProg.map(c => c.title).join(', ') || 'None'}\n\n` +
        `Question: "${userMsg}"\n\n` +
        'Answer in 2-3 sentences. Be friendly, use emojis. Same language as question.';

      const result = await askGroq(prompt, 'You are a helpful learning assistant.');
    ...
    <Box sx={{ p: 1 }}>
      ...
      <Paper elevation={0} sx={{ p: 4, borderRadius: 4,
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Box>
            <Typography variant="h4" sx={{ fontWeight: 900, color: '#fff' }}>AI Personal Insights</Typography>
            <Typography variant="body1" sx={{ opacity: 0.9, color: '#fff' }}>
              Powered by AI — personalized just for you, {userName} 🚀
            </Typography>
          </Box>
        </Stack>
      </Paper>
      ...
      <Paper sx={{ p: 3, borderRadius: 3, mb: 3, border: `2px solid ${MAROON_LIGHT}`, bgcolor: 'rgba(255,255,255,0.6)' }}>
        ...
        <LinearProgress variant="determinate" value={progressPercent}
          sx={{ height: 12, borderRadius: 6, bgcolor: MAROON_LIGHT,
            '& .MuiLinearProgress-bar': { bgcolor: MAROON }, mb: 1 }} />
        <Stack direction="row" justifyContent="space-between">
          <Typography variant="caption" sx={{ fontWeight: 700 }}>{xp} XP</Typography>
          {currentRank.next
            ? <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                {xpToNextRank} XP to reach <strong>{currentRank.next}</strong>
              </Typography>
          ...
          <Stack spacing={3}>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, border: '2px solid rgba(66,133,244,0.3)', bgcolor: 'rgba(66,133,244,0.03)' }}>
              ...
              {aiLoading ? (
                <Stack direction="row" spacing={2} alignItems="center">
                  ...
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>Analyzing your profile...</Typography>
                </Stack>
              ) : (
                <>
                  <Typography variant="body2" sx={{ lineHeight: 1.9, whiteSpace: 'pre-wrap' }}>{aiRecommendation}</Typography>
                  <Button size="small" onClick={generateAIRecommendation}
                    sx={{ mt: 1.5, color: '#4285F4', fontWeight: 700 }}
            ...
            <Paper sx={{ p: 3, borderRadius: 3, border: `2px solid ${MAROON_LIGHT}`, bgcolor: 'rgba(255,255,255,0.6)' }}>
              ...
              <Stack direction="row" spacing={1}>
                <TextField fullWidth size="small" placeholder="Ask me anything about your learning..."
        ...
        <Grid item xs={12} md={5}>
          <Stack spacing={3}>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, border: `2px solid ${MAROON_LIGHT}`, bgcolor: 'rgba(255,255,255,0.6)' }}>
              ...
              <Stack spacing={1.5}>
                {[
                  ['Total XP',          <Typography variant="body2" sx={{ fontWeight: 900, color: MAROON }}>{xp} XP</Typography>],
                  ['Courses completed', <Typography variant="body2" sx={{ fontWeight: 900 }}>{completedCourses.length}</Typography>],
                  ['In progress',       <Typography variant="body2" sx={{ fontWeight: 900 }}>{inProgressCourses.length}</Typography>],
                  ['Current rank',      <Chip label={rankName} size="small" sx={{ bgcolor: '#FACA07', color: '#000', fontWeight: 800 }} />],
                ].map(([label, val]) => (
