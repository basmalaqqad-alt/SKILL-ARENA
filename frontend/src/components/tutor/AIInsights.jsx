// src/components/tutor/AIInsights.jsx
// Groq بدل Gemini — بدون quota
...
import {
  BarChart2, TrendingUp, Users, AlertCircle, Star,
  BookOpen, DollarSign, Award, Zap, BrainCircuit, Sparkles, Send,
} from 'lucide-react';
...
const StatCard = ({ icon: Icon, title, value, subtitle, color = MAROON }) => (
  <Paper sx={{ p: 2.5, borderRadius: 3, border: `2px solid ${MAROON_LIGHT}`,
    bgcolor: 'rgba(255,255,255,0.7)', display: 'flex', alignItems: 'center', gap: 2 }}>
...
const AIInsights = () => {
  ...
  const [chatMessages, setChatMessages] = useState([{
    role: 'ai',
    text: "Hi! 👋 I'm your AI teaching assistant. Ask me anything about your students or courses!",
  }]);
  ...
  }, [courses, students]); // eslint-disable-line
  ...
  const buildContext = () =>
    courses.map(course => {
      ...
      const avgProgress = enrolled.length > 0
        ? Math.round(enrolled.reduce((sum, s) => {
            const c = s.courses.find(c => c.course_id === course.id);
  ...
  const generateAIAnalysis = async () => {
    ...
    try {
      ...
      const prompt =
        'You are an AI teaching advisor for SkillArena platform.\n\n' +
        'Tutor data:\n' +
        '- Total courses: ' + courses.length + '\n' +
        '- Total students: ' + students.length + '\n' +
        '- Courses:\n' +
        courseStats.map(c =>
          `  * "${c.title}": ${c.enrolledCount} students, ${c.completionRate}% completion, avg progress ${c.avgProgress}%`
        ).join('\n') + '\n\n' +
        'Give a SHORT insightful analysis (4-5 sentences):\n' +
        '1. What is going well\n2. What needs attention\n3. One actionable tip\n\n' +
        'Be direct, data-driven, use emojis. English only.';

      const result = await askGroq(prompt, 'You are a helpful teaching analytics AI.');
      setAiAnalysis(result);
    } catch {
      setAiAnalysis('Unable to load AI analysis. Please try again.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const courseStats = buildContext();
      const prompt =
        'You are a helpful AI teaching assistant on SkillArena.\n\n' +
        'Tutor context:\n' +
        '- Courses: ' + (courses.map(c => c.title).join(', ') || 'None') + '\n' +
        '- Students: ' + students.length + '\n' +
        '- Stats: ' + courseStats.map(c => `${c.title}: ${c.enrolledCount} students, ${c.completionRate}% completion`).join(' | ') + '\n\n' +
        'Question: "' + userMsg + '"\n\n' +
        'Answer helpfully in 2-3 sentences. Be friendly, use emojis. Same language as question.';

      const result = await askGroq(prompt, 'You are a helpful teaching assistant.');
  ...
  const needsWork     = [...courseStats].filter(c => c.enrolledCount > 0).sort((a, b) => a.completionRate - b.completionRate)[0];
          ...
          <Stack spacing={3}>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, border: '2px solid rgba(66,133,244,0.3)', bgcolor: 'rgba(66,133,244,0.03)' }}>
              <Typography variant="h6" sx={{ fontWeight: 900, mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Chip label="AI ✨" size="small" sx={{ bgcolor: 'rgba(66,133,244,0.1)', color: '#4285F4', fontWeight: 700, ml: 1 }} />
              </Typography>
              {aiLoading ? (
                <Stack direction="row" spacing={2} alignItems="center">
                  <CircularProgress size={20} sx={{ color: '#4285F4' }} />
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>Analyzing your data...</Typography>
                </Stack>
              ) : (
                <>
                  <Typography variant="body2" sx={{ lineHeight: 1.9, whiteSpace: 'pre-wrap' }}>{aiAnalysis}</Typography>
                  <Button size="small" onClick={generateAIAnalysis} sx={{ mt: 1.5, color: '#4285F4', fontWeight: 700 }}
            ...
            <Paper sx={{ p: 3, borderRadius: 3, border: `2px solid ${MAROON_LIGHT}`, bgcolor: 'rgba(255,255,255,0.6)' }}>
              ...
              {courseStats.length === 0 ? (
                <Typography variant="body2" sx={{ color: 'text.secondary', fontStyle: 'italic' }}>
                  No courses yet. Publish your first course to see analytics!
                </Typography>
              ) : (
                <Stack spacing={2.5}>
                    ...
                    <Box key={course.id}>
                      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 0.5 }}>
                        ...
                        <Stack direction="row" spacing={1}>
                          <Chip label={`${course.enrolledCount} students`} size="small"
                            sx={{ bgcolor: MAROON_LIGHT, color: MAROON, fontWeight: 700 }} />
                          <Chip label={`${course.completionRate}% done`} size="small"
                            sx={{ bgcolor: course.completionRate >= 70 ? 'rgba(46,125,50,0.1)' : 'rgba(237,108,2,0.1)',
                              color: course.completionRate >= 70 ? '#2e7d32' : '#ed6c02', fontWeight: 700 }} />
                        </Stack>
                      </Stack>
                      <LinearProgress variant="determinate" value={course.avgProgress}
            ...
            <Paper sx={{ p: 3, borderRadius: 3, border: `2px solid ${MAROON_LIGHT}`, bgcolor: 'rgba(255,255,255,0.6)' }}>
                ...
                {chatMessages.map((msg, idx) => (
                  <Box key={idx} sx={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                    <Box sx={{ maxWidth: '80%', p: 1.5, borderRadius: 2,
                      bgcolor: msg.role === 'user' ? MAROON : 'rgba(66,133,244,0.08)',
              ...
              <Stack direction="row" spacing={1}>
                <TextField fullWidth size="small" placeholder="Ask about your students or courses..."
              ...
              <Paper sx={{ p: 2.5, borderRadius: 3, border: '2px solid rgba(237,108,2,0.2)', bgcolor: 'rgba(237,108,2,0.04)' }}>
                <Stack direction="row" spacing={1.5} alignItems="flex-start">
                  ...
                  <Box>
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 700 }}>"{needsWork.title}"</Typography>
                    <Typography variant="caption" sx={{ color: 'text.secondary' }}>Only {needsWork.completionRate}% completion.</Typography>
                  </Box>
                </Stack>
              </Paper>
            )}
                    ...
                    <>
                      ...
                      <Stack spacing={0.5}>
                        {stuckStudents.slice(0, 3).map(s => (
                          <Chip key={s.id}
                            avatar={<Avatar sx={{ bgcolor: MAROON, fontSize: '0.7rem' }}>{s.username.charAt(0).toUpperCase()}</Avatar>}
                            label={s.username} size="small"
                            sx={{ bgcolor: MAROON_LIGHT, fontWeight: 700, justifyContent: 'flex-start' }} />
                        ))}
                        {stuckStudents.length > 3 && (
                          <Typography variant="caption" sx={{ color: 'text.secondary' }}>+{stuckStudents.length - 3} more</Typography>
                        )}
                      </Stack>
                    </>
                  )}
                ...
                <Stack direction="row" spacing={1.5} alignItems="flex-start">
                  ...
                  <Box>
                    ...
                    {paidCourses.map(c => {
                      ...
                      return (
                        <Typography key={c.id} variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                          {c.title}: {stat?.enrolledCount || 0} × SAR {Number(c.price)} = SAR {((stat?.enrolledCount || 0) * Number(c.price)).toFixed(0)}
                        </Typography>
                      );
                    })}
                  </Box>
